...
module sd_test_top (
    ...
    // Small FIFO: 64 x 32-bit, single-clock (clk_sd).
    // Only needs to absorb a few values ahead of button presses.
    // Registered read output so Vivado infers Block RAM (1 BRAM).
    (* ram_style = "block" *) reg [31:0] fifo_mem [0:63];
    reg [5:0] fifo_wr_ptr = 6'd0;
    reg [5:0] fifo_rd_ptr = 6'd0;
    wire      fifo_empty = (fifo_wr_ptr == fifo_rd_ptr);
    wire      fifo_full  = (fifo_wr_ptr + 6'd1 == fifo_rd_ptr);
